...
async def create_top_10(request_data, normal):
	...
	if int(course_id) <= 31: #normal 32 courses
		...
		returned_packet = minidom.Document()
		returned_packet.toxml(encoding="UTF-8")
		xml = returned_packet.createElement('RankingDataResponse') #xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns="http://gamespy.net/RaceService/"')			xml.setAttribute('xmlns:xsi', "http://www.w3.org/2001/XMLSchema-instance")
		xml.setAttribute('xmlns:xsd', "http://www.w3.org/2001/XMLSchema")
		xml.setAttribute('xmlns', "http://gamespy.net/RaceService/")
		returned_packet.appendChild(xml)
		responseCode = returned_packet.createElement('responseCode')
		...
		for i, ghost in enumerate(ghosts):
			encode = raceutil.create_base64_encode(ghost, raceutil.match_flag_with_country_code(flags[i]), "", 0, raceutil.channel_time_parse(times[i]), times[i], 0)
			userdatas.append(encode)
